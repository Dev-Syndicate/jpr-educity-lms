...
| B-2 | Search the catalogue by title, author, ISBN or **call number**. |
...
| B-8a | A title also carries a **Call No** — the Dewey classification printed at the left of the rack label ("530" for physics, "420" for English, "512.943 4" for matrices). It sits on the **title**, not the copy: it describes the subject of the work, so every copy of one title shares it, and typing it per copy would only let them drift apart. It is optional, free text (Dewey numbers carry decimals and spaces), and appears **with** the shelf address wherever a location is shown — "530 · Row 09 · Rack 01 · Sec A" — because that is the order the label reads. Searching the catalogue by call number finds every title in that class. |
| B-10 | A title records how it was **acquired**: **invoice number**, **invoice date**, **distributor**, **price** (₹) and **total pages**. All are optional — a donated or inherited item has no invoice, and a title can be catalogued before its paperwork is filed. `books.price` is the **invoiced purchase price** and is a different figure from `book_copies.price`, which is the replacement charge levied when a copy is lost. |
| B-11 | A **Project** or **Thesis** records a **project number**, the **students** who submitted it, and their **department**, **degree** and **batch** (a month and year, e.g. "May 2025"). Department, degree and batch belong to the project as a whole — a project is submitted by one class in one batch — while each student carries their own **roll number** and **name**. A project may have **one student or several**; the form adds and removes student rows as needed rather than fixing a number. **At least one student is required**, because the first student stands as the title's author. |
| B-11a | A project or thesis has **only** these fields. It is a student submission, not a purchase, so the form does not show — and the record does not keep — ISBN, publisher, edition, year, language, description, or any **acquisition** detail (B-10). Those boxes are not rendered rather than shown empty, and switching an existing title's category **clears** the fields that no longer apply, in both directions: re-categorising a book into a project drops its ISBN and invoice; re-categorising a project into a book drops its project number and student list. A stray project number on a magazine is refused by the database outright, not merely hidden. |
| B-11b | A project has **no separate Author field** — its authors are its students. `books.author` is filled from the **first student**, so the catalogue list, counter search and every loan screen keep working unchanged. |
...
| M-1c | Members can be **imported in bulk from a CSV file** (up to 500 rows), with a downloadable template naming the exact columns. `full_name`, `email`, `roll_number` and `member_type` are required; `department`, `phone` and `address` are optional. `faculty` and `staff` are both accepted for the type. Every imported member gets one shared temporary password, set on the import screen, which they change themselves under Password. **Partial by design:** valid rows are created and failed rows are reported with their line number and reason, so one bad row does not cost the rest of the file. Re-importing a corrected file is safe — rows that already exist are rejected as duplicates rather than created twice. |
...
- **`books`** — title level: title, author, isbn, publisher, year, `category` (enum: `book` · `non_book_material` · `project` · `thesis` · `proceeding` · `magazine`), `department` (free text), `call_no` (the Dewey classification from the rack label). Not to be confused with `profiles.department`, which is the member's own. Acquisition lives here too — `invoice_no`, `invoice_date`, `distributor`, `price`, `total_pages` — as does the project/thesis header: `project_no`, `degree`, `batch_month`. `books.price` is the invoiced price; `book_copies.price` is the replacement charge.
- **`project_authors`** — the students behind a project or thesis, one row each: `roll_number`, `full_name`, `position`. `on delete cascade` from `books`, since an author list has no meaning without its project. Department, degree and batch are deliberately *not* here — they describe the project, not each student.
